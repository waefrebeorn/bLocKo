...
def key_to_string(key, is_gamepad=False):
    """Convert a key code to a readable string."""
    key_map = {
        arcade.key.UP: "Up",
        ...
        arcade.key.LSHIFT: "Left Shift",
        arcade.key.RSHIFT: "Right Shift",
        arcade.key.LCTRL: "Left Ctrl",
        arcade.key.RCTRL: "Right Ctrl",
        arcade.key.LALT: "Left Alt",
        arcade.key.RALT: "Right Alt",
        arcade.key.Z: "Z",
        arcade.key.X: "X",
        arcade.key.C: "C",
        arcade.key.P: "P"
    ...
    if is_gamepad:
        return GAMEPAD_BUTTON_MAPPINGS.get(key, f"Button {key}")
    
    return key_map.get(key, f"Key {chr(key).upper()}" if isinstance(key, int) else f"Key {key}")
...
class Block:
    ...
    def rotate(self, clockwise, grid):
        ...
        if clockwise:
            self.shape = [(-y + center_y + center_x, x - center_x + center_y) for x, y in self.shape]
        ...
        kick_set = WALL_KICK_OFFSETS['I' if self.block_type == 'I' else 'non-I'][original_state]
        for kick in kick_set:
            test_x = self.grid_x + kick[0]
            test_y = self.grid_y + kick[1]
            if self.is_valid_position([(x + test_x, y + test_y) for x, y in self.shape], grid):
                self.grid_x = test_x
                self.grid_y = test_y
                return True
    ...
    def is_valid_position(self, positions, grid):
        for x, y in positions:
            if x < 0 or x >= GRID_WIDTH or y < 0 or y >= GRID_HEIGHT or (y < GRID_HEIGHT and grid[y][x]):
                return False
        return True
...
class BKGame(arcade.Window):
    def __init__(self):
        ...
        self.setup()  # Initialize all game variables
        self.game_state = GameState.MAIN_MENU  # Start in the main menu
        self.bg_music = None
                ...
                with open("key_bindings.json", "r") as f:
                    ...
                    for action, key in bindings.items():
                        if isinstance(key, str):
                            key_value = getattr(arcade.key, key.upper(), None)
                            if key_value:
                                bindings[action] = key_value
                    return bindings
            except json.JSONDecodeError:
    ...
    def save_key_bindings(self):
        ...
        for action, key in self.key_bindings.items():
            key_name = arcade.key.NAME[key] if hasattr(arcade.key, 'NAME') and key in arcade.key.NAME else key
            bindings_to_save[action] = key_name
        with open("key_bindings.json", "w") as f:
            json.dump(bindings_to_save, f)

    def get_ghost_position(self):
        """Calculate and return the ghost block's position."""
        if not self.current_block:
            return []
    
        # Create a copy of the current block to simulate the ghost block
        ghost_block = Block(
        ...
        # Move the ghost block down until it reaches a valid position
    ...
    def spawn_new_block(self):
        ...
        self.current_block.grid_y = GRID_HEIGHT + 1
    ...
    def is_valid_position(self, positions):
        return all(0 <= x < GRID_WIDTH and 0 <= y < GRID_HEIGHT and (y < GRID_HEIGHT and self.grid[y][x] is None) for x, y in positions)
    ...
    def settle_column(self, x):
        ...
        settled_column = [None] * (GRID_HEIGHT - len(settled_column)) + settled_column
    ...
    def trigger_bomb(self):
        ...
        for y in range(max(0, bomb_y - 2), min(GRID_HEIGHT, bomb_y + 3)):
    ...
    def handle_menu_selection(self):
        if self.game_state == GameState.MAIN_MENU:
            ...
            if selected == "Back":
                ...
                self.game_mode = self.mode_selection
                self.start_game()
        elif self.game_state == GameState.OPTIONS:
    ...
    def handle_playing_input(self, key):
        if self.current_block:
            if key == self.key_bindings["MOVE_LEFT"]:
                ...
                if self.current_block.rotate(False, self.grid):
                    arcade.play_sound(ROTATE_SOUND)
            elif key == self.key_bindings["ROTATE_RIGHT"]:
                if self.current_block.rotate(True, self.grid):
                    arcade.play_sound(ROTATE_SOUND)
    ...
    def handle_key_rebinding(self, key):
        if self.rebinding_action:
            ...
            # Start rebinding the selected key
            if key == self.key_bindings["MOVE_UP"]:
                self.menu_selection = (self.menu_selection - 1) % len(self.key_bindings)
            elif key == self.key_bindings["MOVE_DOWN"]:
                self.menu_selection = (self.menu_selection + 1) % len(self.key_bindings)
    ...
    def on_key_release(self, key, modifiers):
        pass  # Currently not handling key releases
    ...
    def add_pressure_blocks(self):
        for y in range(GRID_HEIGHT - 1, self.pressure_height - 1, -1):
    ...
    def handle_menu_back(self):
        """Handles going back in the menu."""
        if self.game_state == GameState.GAME_MODE_SELECT or self.game_state == GameState.OPTIONS:
            self.game_state = GameState.MAIN_MENU
        elif self.game_state == GameState.KEY_BINDING:
            self.game_state = GameState.OPTIONS
        elif self.game_state == GameState.TUTORIAL:
    ...
    def handle_menu_selection(self):
        """Handles selecting an option in the current menu."""
        if self.game_state == GameState.MAIN_MENU:
    ...
    def handle_menu_down(self):
        """Handles moving down in the menu."""
        if self.game_state == GameState.MAIN_MENU:
            self.menu_selection = (self.menu_selection + 1) % 4
        elif self.game_state == GameState.GAME_MODE_SELECT:
            self.mode_selection = (self.mode_selection + 1) % 5
    ...
    def on_key_press(self, key, modifiers):
        """Handles key presses for navigation and gameplay."""
        if self.game_state == GameState.KEY_BINDING:
            self.handle_key_rebinding(key)
        elif self.game_state == GameState.MAIN_MENU:
            self.handle_main_menu_input(key)
    ...
    def handle_game_mode_select_input(self, key):
        """Handles input for the game mode select menu."""
        if key == self.key_bindings["MOVE_UP"]:
            self.mode_selection = (self.mode_selection - 1) % 5
        elif key == self.key_bindings["MOVE_DOWN"]:
            self.mode_selection = (self.mode_selection + 1) % 5
    ...
    def handle_options_input(self, key):
        ...
        if key == self.key_bindings["MOVE_UP"]:
            ...
            if self.option_selection == 0:
                self.power_ups_enabled = not self.power_ups_enabled
            elif self.option_selection == 1:
                self.difficulty = (self.difficulty % 3) + 1
            elif self.option_selection == 2:
                self.game_state = GameState.KEY_BINDING
                self.menu_selection = 0
            elif self.option_selection == 3:
                self.game_state = GameState.MAIN_MENU
        elif key == self.key_bindings["BACK"]:
            self.game_state = GameState.MAIN_MENU
    ...
    def handle_key_rebinding(self, key):
        """Handles key rebinding in the options menu."""
        if self.rebinding_action:
            if key != arcade.key.ESCAPE:
                self.key_bindings[self.rebinding_action] = key
                self.save_key_bindings()
